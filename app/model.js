var home = {
  list: [
    {
      title: "dimas",
      link: "/home",
      img: "#",
      content:
        "anpfanp apfnpanfpf panapwfnp poapfpfpadasdasdas asdasdas  asd asda sd asdasdasdasd. asd asd as. sa asdasdas, asdasdasd,sad   . sasdasdasd  apaso"
    },
    {
      title: "dimas",
      link: "/home",
      img: "#",
      content:
        "anpfanp apfnpanfpf panapwfnp poapfpfpadasdasdas asdasdas  asd asda sd asdasdasdasd. asd asd as. sa asdasdas, asdasdasd,sad   . sasdasdasd  apaso"
    }
  ],
  info: [
    {
      title: "satu",
      fill:
        "dasdasd asda sdas dasd asd asd a aeh eg ea fae fef  ef aef ae  aef af awd ad d ad awd awd awd g aw aw g w ad awdawdawdawd dawd awd wd awdawdawdadw aw dawd awd  awdawdawd wad awd ad awd wdaw d adawdawdaw dad awd awd awd e"
    },
    {
      title: "dua",
      fill:
        "dasdasd asda sdas dasd asd asd a aeh eg ea fae fef  ef aef ae  aef af awd ad d ad awd awd awd g aw aw g w ad awdawdawdawd dawd awd wd awdawdawdadw aw dawd awd  awdawdawd wad awd ad awd wdaw d adawdawdaw dad awd awd awd e"
    },
    {
      title: "tiga",
      fill:
        "dasdasd asda sdas dasd asd asd a aeh eg ea fae fef  ef aef ae  aef af awd ad d ad awd awd awd g aw aw g w ad awdawdawdawd dawd awd wd awdawdawdadw aw dawd awd  awdawdawd wad awd ad awd wdaw d adawdawdaw dad awd awd awd e"
    }
  ],
  update: [
    {
      title: "satu",
      unit:
        "adawdawd awda da wda wda dad  gr sr gsr ggsrgsg srg srgsr g  hdt thd th dt thm aerhawegawge a ef awf w f wafafawfaf aw fawf awf af wafawf w fw awf",
      url: "update-satu"
    },
    {
      title: "dua",
      unit:
        "adawdawd awda da wda wda dad  gr sr gsr ggsrgsg srg srgsr g  hdt thd th dt thm aerhawegawge a ef awf w f wafafawfaf aw fawf awf af wafawf w fw awf",
      url: "update-dua"
    },
    {
      title: "tiga",
      unit:
        "adawdawd awda da wda wda dad  gr sr gsr ggsrgsg srg srgsr g  hdt thd th dt thm aerhawegawge a ef awf w f wafafawfaf aw fawf awf af wafawf w fw awf",
      url: "update-satu3"
    },
    {
      title: "empat",
      unit:
        "adawdawd awda da wda wda dad  gr sr gsr ggsrgsg srg srgsr g  hdt thd th dt thm aerhawegawge a ef awf w f wafafawfaf aw fawf awf af wafawf w fw awf",
      url: "update-empat"
    },
    {
      title: "lima",
      unit:
        "adawdawd awda da wda wda dad  gr sr gsr ggsrgsg srg srgsr g  hdt thd th dt thm aerhawegawge a ef awf w f wafafawfaf aw fawf awf af wafawf w fw awf",
      url: "update-lima"
    },
    {
      title: "enam",
      unit:
        "adawdawd awda da wda wda dad  gr sr gsr ggsrgsg srg srgsr g  hdt thd th dt thm aerhawegawge a ef awf w f wafafawfaf aw fawf awf af wafawf w fw awf",
      url: "update-enam"
    },
    {
      title: "tujuh",
      unit:
        "adawdawd awda da wda wda dad  gr sr gsr ggsrgsg srg srgsr g  hdt thd th dt thm aerhawegawge a ef awf w f wafafawfaf aw fawf awf af wafawf w fw awf",
      url: "update-tujuh"
    },
    {
      title: "delapan",
      unit:
        "adawdawd awda da wda wda dad  gr sr gsr ggsrgsg srg srgsr g  hdt thd th dt thm aerhawegawge a ef awf w f wafafawfaf aw fawf awf af wafawf w fw awf",
      url: "update-delapan"
    }
  ]
};
var nav = {
  sign: [
    {
      name: "pendaftaran",
      url: "daftar()"
    },
    {
      name: "mahasiswa",
      url: "chkm()"
    },
    {
      name: "dosen",
      url: "chkd()"
    },
    {
      name: "alumni",
      url: "alm()"
    },
    {
      name: "orang tua",
      url: "ortu()"
    }
  ],
  menu: [
    {
      name: "beranda",
      url: "home()"
    },
    {
      name: "profil",
      url: "profil()"
    },
    {
      name: "akademik",
      url: "akademik()"
    },
    {
      name: "prodi",
      url: "prodi()"
    },
    {
      name: "AWARD",
      url: "award()"
    },
    {
      name: "organisasi",
      url: "org()"
    }
  ]
};
var akademik = {
  daftar: [
    {
      name: "diploma",
      isi: [
        {
          prodi: "Manajemen Informatika",
          sks: "62",
          semester: "genap",
          tahun: "2018/2019",
          url: "akademik(d3mi)"
        },
        {
          prodi: "Teknik Informatika",
          sks: "110",
          semester: "genap",
          tahun: "2018/2019",
          url: "akademik(d3ti)"
        }
      ]
    },
    {
      name: "Sarjana",
      isi: [
        {
          prodi: "Sistem Informasi",
          sks: "80",
          semester: "genap",
          tahun: "2018/2019",
          url: "akademik(s1si)"
        },
        {
          prodi: "Teknik Informatika",
          sks: "96",
          semester: "genap",
          tahun: "2018/2019",
          url: "akademik(s1if)"
        },
        {
          prodi: "SI Transfer",
          sks: "80",
          semester: "genap",
          tahun: "2018/2019",
          url: "akademik(s1sit)"
        },
        {
          prodi: "IF Transfer",
          sks: "96",
          semester: "genap",
          tahun: "2018/2019",
          url: "akademik(s1ift)"
        }
      ]
    }
  ],
  info: [
    {
      title: "ini berita pertama",
      tanggal: "02 januari 2019",
      url: "berita(1)"
    },
    {
      title: "ini berita kedua",
      tanggal: "24 Februari 2019",
      url: "berita(2)"
    },
    {
      title: "ini berita ketiga",
      tanggal: "06 April 2019",
      url: "berita(3)"
    }
  ],
  kampus: [1, 2, 3, 4, 5, 6, 7, 8],
  jadwal: [
    {
      name: "Jadwal Kuliah",
      url: "http://www.amikom.ac.id/index.php/akademik/jadwal/kuliah"
    },
    {
      name: "Jadwal Ujian",
      url: "hhttp://www.amikom.ac.id/index.php/akademik/jadwal/ujian"
    },
    {
      name: "Kalender Akademik 2018/2019",
      url:
        "http://www.amikom.ac.id/public/download/kalendar_akademik_2018_2019.pdf"
    },
    {
      name: "Kalender Akademik 2019/2020",
      url:
        "http://www.amikom.ac.id/public/download/kalender_akademik_2019-2020.pdf"
    }
  ]
};
var semester = ["Ganjil", "Genap", "Remidial Ganjil", "Remidial Genap"];
var tahun = [
  "2019/2018",
  "2018/2017",
  "2017/2016",
  "2016/2015",
  "2015/2014",
  "2014/2013",
  "2013/2012"
];
var akun = {
  dosen: [
    {
      nama: "I Made Artha Agastya, S.T., M.Eng",
      email: "artha.agastya",
      sinopsis: "http://artha.agastya.ac.id",
      pics: "/images/dosen/imade.png"
    },
    {
      nama: "Nuri Cahyono, M.Kom",
      email: "nuricahyono",
      sinopsis: "Live Your Dream",
      pics: "/images/dosen/nuri.png",
      materi: [
        {
          judul: 1,
          semester: "Ganjil",
          sinopsis: "Materi tentang penelitian",
          download: 23,
          matkul: "Struktur Data",
          thn: "2019/2018",
          date: "06-02-2019"
        },
        {
          judul: 2,
          semester: "Ganjil",
          sinopsis: "lanjutan materi",
          download: 12,
          matkul: "Struktur Data",
          thn: "2019/2018",
          date: "13-02-2019"
        },
        {
          judul: 3,
          semester: "Ganjil",
          sinopsis: "sebelum uts",
          download: 5,
          matkul: "Sistem Operasi",
          thn: "2018/2017",
          date: "12-09-2018"
        },
        {
          judul: 1,
          semester: "Genap",
          sinopsis: "Materi tentang penelitian",
          download: 32,
          matkul: "pemrograman",
          thn: "2019/2018",
          date: "24-01-2019"
        },
        {
          judul: 2,
          semester: "Genap",
          sinopsis: "tentang OOP",
          download: 37,
          matkul: "pemrograman",
          thn: "2018/2017",
          date: "08-12-2017"
        },
        {
          judul: 1,
          semester: "Remidial Ganjil",
          sinopsis: "Materi mengulang",
          download: 9,
          matkul: "Struktur Data",
          thn: "2019/2018",
          date: "06-11-2018"
        },
        {
          judul: 2,
          semester: "Remidial Genap",
          sinopsis: "Materi susulan",
          download: 14,
          matkul: "Pemrograman Web",
          thn: "2019/2018",
          date: "11-10-2018"
        },
        {
          judul: 1,
          semester: "Ganjil",
          sinopsis: "Materi tentang penelitian",
          download: 23,
          matkul: "Struktur Data",
          thn: "2017/2016",
          date: "10-09-2016"
        },
        {
          judul: 2,
          semester: "Ganjil",
          sinopsis: "Lanjutan..",
          download: 26,
          matkul: "Struktur Data",
          thn: "2017/2016",
          date: "17-09-2016"
        }
      ],
      jadwal: [
        {
          hari: "SENIN",
          jam: "10.40 - 12.20",
          matkul: "PEMROGRAMAN",
          kelas: "18-S1IF-04",
          ruang: "L 7.4.1"
        },
        {
          hari: "SENIN",
          jam: "13.20 - 15.00	",
          matkul: "PEMROGRAMAN",
          kelas: "18-S1IF-06",
          ruang: "L 7.4.1"
        },
        {
          hari: "SENIN",
          jam: "15.30 - 17.10",
          matkul: "PEMROGRAMAN",
          kelas: "18-S1IF-01",
          ruang: "L 7.4.1"
        },
        {
          hari: "SELASA",
          jam: "15.30 - 17.10",
          matkul: "PEMROGRAMAN",
          kelas: "18-S1IF-02",
          ruang: "L 7.4.1"
        },
        {
          hari: "RABU",
          jam: "07.00 - 08.40	",
          matkul: "STRUKTUR DATA",
          kelas: "18-S1IF-11",
          ruang: "04.04.02"
        },
        {
          hari: "RABU",
          jam: "10.40 - 12.20",
          matkul: "PEMROGRAMAN",
          kelas: "18-S1IF-05",
          ruang: "L 7.4.1"
        },
        {
          hari: "KAMIS",
          jam: "13.20 - 15.00",
          matkul: "PEMROGRAMAN",
          kelas: "18-S1IF-03",
          ruang: "L 7.4.1"
        },
        {
          hari: "SABTU",
          jam: "10.40 - 12.20",
          matkul: "PEMROGRAMAN",
          kelas: "18-BCI-01",
          ruang: "L 6.2.1"
        },
        {
          hari: "SABTU",
          jam: "14.20 - 16.00",
          matkul: "STRUKTUR DATA",
          kelas: "18-S1IF-11",
          ruang: "L 7.3.2"
        }
      ],
      nilai: [
        {
          title: "REKAYASA PERANGKAT LUNAK",
          prodi: "Informatika",
          isi: [
            {
              judul: "Nilai 190302332 - ST063_7_2018/2019",
              tanggal: "13-02-2019 (09:18:00)"
            },
            {
              judul: "Nilai 190302332 - ST063_7_2018/2019",
              tanggal: "08-02-2019 (12:48:00)"
            }
          ]
        },
        {
          title: "PEMROGRAMAN LANJUT",
          prodi: "Informatika",
          isi: [
            {
              judul: "Nilai 190302332 - ST122_7_2018/2019",
              tanggal: "13-02-2019 (09:18:00)"
            },
            {
              judul: "Nilai 190302332 - ST122_7_2018/2019",
              tanggal: "06-02-2019 (10:50:00)"
            }
          ]
        }
      ]
    },
    {
      nama: "Rakhma Safrida Kurnia, S.Kom., M.Kom",
      email: "rakhmakurnia",
      sinopsis: "Serves as lecture in Amikom Yogyakarta",
      pics: "/images/dosen/rakhma.png"
    },
    {
      nama: "Junaidi, S.Ag., M.Hum",
      email: "Junaidi.i",
      sinopsis: "BERKOMITMEN MENCERDASKAN ANAK BANGSA INDONESIA ",
      pics: "/images/dosen/junaidi.png"
    },
    {
      nama: "Krisnawati, S.Si., M.T.",
      email: "krisna",
      sinopsis: "Suka MEMBACA",
      pics: "/images/dosen/krisna.png"
    },
    {
      nama: "Kumara Ari Yuana,S.T, M.T",
      email: "kumara.a",
      sinopsis: "The Greatest Philosophers adalah buku yang ditulis Kumara ",
      pics: "/images/dosen/kumara.png"
    },
    {
      nama: "M. Rudyanto Arief, S.T, M.T",
      email: "rudy",
      sinopsis:
        "Hobby : - Sports (Badminton & Jogging) - Music - Surfing and Browsing - Chatting ^_^",
      pics: "/images/dosen/rudyanto.png"
    },
    {
      nama: "Lia Ayu Ivanjelita, M.Kom",
      email: "liaayu",
      sinopsis: "Agama = Islam",
      pics: "/images/dosen/lia.png"
    },
    {
      nama: "Yoga Pristyanto, S.Kom, M.Eng",
      email: "yoga.pristyanto",
      sinopsis: "https://www.researchgate.net/profile/Yoga_Pristyanto",
      pics: "/images/dosen/yoga.png"
    },
    {
      nama: "Wing Wahyu Winarno, Dr., MAFIS., Ak.",
      email: "wing",
      sinopsis: " maswing.wordpress.com",
      pics: "/images/dosen/wing.png"
    },
    {
      nama: "Hendra Kurniawan, M.Kom",
      email: "hendrakurniawan",
      sinopsis:
        "Indeed, my prayer, my rites of sacrifice, my living and my dying are for Allah, Lord of the worlds.",
      pics: "/images/dosen/hendra.png"
    },
    {
      nama: "Ainul Yaqin, M. Kom",
      email: "ainulyaqin",
      sinopsis:
        "Semua punya kesempatan yang sama untuk menuju kesuksesan, tinggal sampai dimana anda bertahan dengan tantangan yang dihadapi untuk menuju",
      pics: "/images/dosen/ainul.png"
    }
  ],
  mhs: [
    {
      user: "17.11.1725",
      name: "dimas",
      pass: "123"
    },
    {
      user: "17.11.1726",
      name: "bista",
      pass: "jkt"
    },
    {
      user: "17.11.1727",
      name: "hara",
      pass: "321"
    }
  ]
};
var award = [
  "Gold Remi Award, Worldfest, Houston, International Film Festival 2016",
  "Grandprize Winner, SICAF 2016, The 20th Seoul International Cartoon and Animation Festival",
  "Winner Best Animation, 3th Noida International Film Festival 2016",
  "Nominee International ICT Innovative Service Contest, Taiwan, 2016",
  "BSmart, Nominee Asia Pacific ICT Award, Taiwan, 2016",
  "Gablind, Nominee Asia Pacific ICT Award, Taiwan, 2016",
  "HICO, Nominee, The use of ICTs in education for disadvantaged groups, UNESCO 2016",
  "Ahada, Nominee, The use of ICTs in education for disadvantaged groups, UNESCO 2016 ",
  "BTOUCH, Nominee, The use of ICTs in education for disadvantaged groups, UNESCO 2016",
  "Official Selection, Holland Animation Film Festival 2016"
];
var prodi = [
  {
    name: "Akuntansi",
    link: "http://akuntansi.amikom.ac.id"
  },
  {
    name: "Arsitektur",
    link: "http://arsitektur.amikom.ac.id"
  },
  {
    name: "Geografi ",
    link: "http://geografi.amikom.ac.id"
  },
  {
    name: "Ilmu Komunikasi",
    link: "http://ilmukomunikasi.amikom.ac.id"
  },
  {
    name: "Informatika",
    link: "http://informatika.amikom.ac.id"
  },
  {
    name: "Magister Teknik Informatika",
    link: "http://mti.amikom.ac.id"
  },
  {
    name: "Perencanaan Wilayah dan kota",
    link: "http://pwk.amikom.ac.id"
  },
  {
    name: "Teknik Informatika",
    link: "http://d3ti.amikom.ac.id"
  },
  {
    name: "Teknologi Informasi ",
    link: "http://ti.amikom.ac.id"
  },
  {
    name: "Ekonomi ",
    link: "http://ekonomi.amikom.ac.id"
  },
  {
    name: "Hubungan Internasional  ",
    link: "http://hi.amikom.ac.id"
  },
  {
    name: "Ilmu Pemerintahan ",
    link: "http://ilmupemerintahan.amikom.ac.id"
  },
  {
    name: "Kewirausahaan ",
    link: "http://kewirausahaan.amikom.ac.id"
  },
  {
    name: "Manajemen Informatika ",
    link: "http://d3mi.amikom.ac.id"
  },
  {
    name: "Sistem Informasi ",
    link: "http://s1si.amikom.ac.id"
  },
  {
    name: "Teknik Komputer ",
    link: "http://tk.amikom.ac.id"
  }
];
var daftar = {
  gel: [
    {
      name: "KHUSUS ",
      from: "03-12-2018",
      till: "06-04-2019"
    },
    {
      name: "I ",
      from: "07-04-2019",
      till: "10-05-2019"
    },
    {
      name: "II ",
      from: "11-05-2019",
      till: "29-06-2019"
    },
    {
      name: "III ",
      from: "30-06-2019",
      till: "23-08-2019"
    }
  ],
  menu: [
    {
      name: "Alur Pendaftaran",
      link: "http://pmb.amikom.ac.id/images/alur.png",
      icon: "1"
    },
    {
      name: "Jadwal PSU",
      link: "http://pmb.amikom.ac.id/index.php/jadwal_psu",
      icon: "2"
    },
    {
      name: "PPM",
      link: "http://ppm.amikom.ac.id/",
      icon: "3"
    },
    {
      name: "Profil",
      link: "http://amikom.ac.id/public/pdf_profile/Profil_2019.pdf",
      icon: "4"
    },
    {
      name: "Denah",
      link: "http://public.amikom.ac.id/pmb/denah_amikom.jpg",
      icon: "5"
    },
    {
      name: "ITC",
      link: "http://itc.amcc.or.id/",
      icon: "6"
    }
  ]
};
var form = {
  single: [
    {
      title: "Nama",
      ph: "Nama Anda",
      type: "text"
    },
    {
      title: "Nomor Identitas (KTP)",
      ph: "Nomor Identitas (KTP)",
      type: "text"
    },
    {
      title: "Kota Kelahiran",
      ph: "Tempat Lahir",
      type: "text"
    },
    {
      title: "Tanggal Lahir",
      ph: "Tanggal Lahir",
      type: "date"
    },
    {
      title: "Kelurahan",
      ph: "Kelurahan",
      type: "text"
    },
    {
      title: "Kecamatan",
      ph: "Tempat Lahir",
      type: "text"
    },
    {
      title: "Kodepos",
      ph: "Kodepos",
      type: "date"
    },
    {
      title: "Nama ibu (Sebelum menikah)",
      ph: "Nama ibu",
      type: "text"
    },
    {
      title: "Sekolah Asal",
      ph: "Sekolah Asal Anda",
      type: "text"
    }
  ],
  multi: [
    {
      title: "Jenis Kelamin",
      fill: [
        {
          title: "--Pilih salah satu--",
          val: 0
        },
        {
          title: "laki-laki",
          val: 1
        },
        {
          title: "perempuan",
          val: 2
        }
      ]
    },
    {
      title: "Agama",
      fill: [
        {
          title: "--Pilih Agama--",
          val: 0
        },
        {
          title: "Islam",
          val: 1
        },
        {
          title: "Katholik",
          val: 2
        },
        {
          title: "Protestan",
          val: 3
        },
        {
          title: "Hindu",
          val: 4
        },
        {
          title: "Budha",
          val: 5
        },
        {
          title: "Lainnya",
          val: 99
        }
      ]
    },
    {
      title: "Kewarganegaraan",
      fill: [
        {
          title: "WNI",
          val: 0
        },
        {
          title: "WNA",
          val: 1
        }
      ]
    },
    {
      title: "provinsi",
      fill: [
        { val: 1, title: "Nanggroe Aceh Darussalam" },
        { val: 2, title: "Sumatera Utara" },
        { val: 3, title: "Sumatera Barat" },
        { val: 4, title: "Riau" },
        { val: 5, title: "Jambi" },
        { val: 6, title: "Sumatera Selatan" },
        { val: 7, title: "Bengkulu" },
        { val: 8, title: "Lampung" },
        { val: 9, title: "Bangka Belitung" },
        { val: 10, title: "Kepulauan Riau" },
        { val: 11, title: "DKI Jakarta" },
        { val: 12, title: "Jawa Barat" },
        { val: 13, title: "Jawa Tengah" },
        { val: 14, title: "Daerah Istimewa Yogyakarta" },
        { val: 15, title: "Jawa Timur" },
        { val: 16, title: "Banten" },
        { val: 17, title: "Bali" },
        { val: 18, title: "Nusa Tenggara Barat" },
        { val: 19, title: "Nusa Tenggara Timur" },
        { val: 20, title: "Kalimantan Barat" },
        { val: 21, title: "Kalimantan Tengah" },
        { val: 22, title: "Kalimantan Selatan" },
        { val: 23, title: "Kalimantan Timur" },
        { val: 24, title: "Sulawesi Utara" },
        { val: 25, title: "Sulawesi Tengah" },
        { val: 26, title: "Sulawesi Selatan" },
        { val: 27, title: "Sulawesi Tenggara" },
        { val: 28, title: "Gorontalo" },
        { val: 29, title: "Maluku" },
        { val: 30, title: "Maluku Utara" },
        { val: 31, title: "Papua Barat" },
        { val: 32, title: "Papua" },
        { val: 33, title: "Luar Negeri" },
        { val: 34, title: "Sulawesi Barat" },
        { val: 38, title: "Kalimantan Utara" },
        { val: 99, title: "Lainnya" }
      ]
    },
    {
      title: "Kabupaten",
      fill: [
        {
          title: "--Pilih provinsi dulu--",
          val: 0
        }
      ]
    },
    {
      title: "Jenjang Pendidikan",
      fill: [
        { val: "", title: "--Pilih Jenjang Pendidikan--" },
        { val: "IPA", title: "SMA - IPA" },
        { val: "IPS", title: "SMA - IPS" },
        { val: "STM", title: "SMK - IPA" },
        { val: "SMEA", title: "SMK - IPS" }
      ]
    },
    {
      title: "Pilihan 1",
      fill: [
        { val: "", title: "--Pilihan 1--" },
        { val: "01", title: "D3-Teknik Informatika" },
        { val: "02", title: "D3-Manajemen Informatika" },
        { val: "11", title: "S1-Informatika (Teknik Informatika)" },
        { val: "12", title: "S1-Sistem Informasi" },
        { val: "60", title: "S1-Bachelor Information Technology" },
        { val: "61", title: "S1-Bachelor Informatics" },
        { val: "62", title: "S1-Bachelor Information System" },
        { val: "67", title: "S1-Bachelor Communication Science" },
        { val: "82", title: "S1-Teknologi Informasi" },
        { val: "83", title: "S1-Teknik Komputer" },
        { val: "84", title: "S1-Arsitektur" },
        { val: "85", title: "S1-Geografi" },
        { val: "86", title: "S1-Perencanaan Wilayah dan Kota" },
        { val: "91", title: "S1-Ekonomi" },
        { val: "92", title: "S1-Kewirausahaan" },
        { val: "93", title: "S1-Akuntansi" },
        { val: "94", title: "S1-Ilmu Pemerintahan" },
        { val: "95", title: "S1-Hubungan Internasional" },
        { val: "96", title: "S1-Ilmu Komunikasi" }
      ]
    },
    {
      title: "Pilihan 2",
      fill: [
        { val: "", title: "--Pilihan 2--" },
        { val: "01", title: "D3-Teknik Informatika" },
        { val: "02", title: "D3-Manajemen Informatika" },
        { val: "11", title: "S1-Informatika (Teknik Informatika)" },
        { val: "12", title: "S1-Sistem Informasi" },
        { val: "60", title: "S1-Bachelor Information Technology" },
        { val: "61", title: "S1-Bachelor Informatics" },
        { val: "62", title: "S1-Bachelor Information System" },
        { val: "67", title: "S1-Bachelor Communication Science" },
        { val: "82", title: "S1-Teknologi Informasi" },
        { val: "83", title: "S1-Teknik Komputer" },
        { val: "84", title: "S1-Arsitektur" },
        { val: "85", title: "S1-Geografi" },
        { val: "86", title: "S1-Perencanaan Wilayah dan Kota" },
        { val: "91", title: "S1-Ekonomi" },
        { val: "92", title: "S1-Kewirausahaan" },
        { val: "93", title: "S1-Akuntansi" },
        { val: "94", title: "S1-Ilmu Pemerintahan" },
        { val: "95", title: "S1-Hubungan Internasional" },
        { val: "96", title: "S1-Ilmu Komunikasi" }
      ]
    },
    {
      title: "Pilihan 3",
      fill: [
        { val: "", title: "--Pilihan 3--" },
        { val: "01", title: "D3-Teknik Informatika" },
        { val: "02", title: "D3-Manajemen Informatika" },
        { val: "11", title: "S1-Informatika (Teknik Informatika)" },
        { val: "12", title: "S1-Sistem Informasi" },
        { val: "60", title: "S1-Bachelor Information Technology" },
        { val: "61", title: "S1-Bachelor Informatics" },
        { val: "62", title: "S1-Bachelor Information System" },
        { val: "67", title: "S1-Bachelor Communication Science" },
        { val: "82", title: "S1-Teknologi Informasi" },
        { val: "83", title: "S1-Teknik Komputer" },
        { val: "84", title: "S1-Arsitektur" },
        { val: "85", title: "S1-Geografi" },
        { val: "86", title: "S1-Perencanaan Wilayah dan Kota" },
        { val: "91", title: "S1-Ekonomi" },
        { val: "92", title: "S1-Kewirausahaan" },
        { val: "93", title: "S1-Akuntansi" },
        { val: "94", title: "S1-Ilmu Pemerintahan" },
        { val: "95", title: "S1-Hubungan Internasional" },
        { val: "96", title: "S1-Ilmu Komunikasi" }
      ]
    },
    {
      title: "Pilihan 4",
      fill: [
        { val: "", title: "--Pilihan 4--" },
        { val: "01", title: "D3-Teknik Informatika" },
        { val: "02", title: "D3-Manajemen Informatika" },
        { val: "11", title: "S1-Informatika (Teknik Informatika)" },
        { val: "12", title: "S1-Sistem Informasi" },
        { val: "60", title: "S1-Bachelor Information Technology" },
        { val: "61", title: "S1-Bachelor Informatics" },
        { val: "62", title: "S1-Bachelor Information System" },
        { val: "67", title: "S1-Bachelor Communication Science" },
        { val: "82", title: "S1-Teknologi Informasi" },
        { val: "83", title: "S1-Teknik Komputer" },
        { val: "84", title: "S1-Arsitektur" },
        { val: "85", title: "S1-Geografi" },
        { val: "86", title: "S1-Perencanaan Wilayah dan Kota" },
        { val: "91", title: "S1-Ekonomi" },
        { val: "92", title: "S1-Kewirausahaan" },
        { val: "93", title: "S1-Akuntansi" },
        { val: "94", title: "S1-Ilmu Pemerintahan" },
        { val: "95", title: "S1-Hubungan Internasional" },
        { val: "96", title: "S1-Ilmu Komunikasi" }
      ]
    }
  ]
};
